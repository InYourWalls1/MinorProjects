...
async def on_message(message):
    ...
    if (tts_channel[message.guild.id] == message.channel.id):
        os.system('del output.wav')
        messageText = message.content
        if messageText[0:3].lower() == "en!":
            messageText = messageText[3:]
            if (previousAuthor == message.author.id):
                subprocess.run(['espeak-ng', '-s', '130', '-w', 'output.wav', messageText], check=True)
            else:
                text = f"{message.author.name} says {messageText}"
                subprocess.run(['espeak-ng', '-s', '130', '-w', 'output.wav', text], check=True)
        else:
            if (previousAuthor == message.author.id):
                subprocess.run(['espeak-ng', '-v', 'it', '-s', '130', '-w', 'output.wav', messageText], check=True)
            else:
                text = f"{message.author.name} dice {messageText}"
                subprocess.run(['espeak-ng', '-v', 'it', '-s', '130', '-w', 'output.wav', text], check=True)
        while not (os.path.isfile("output.wav")):
            time.sleep(1)
            print("taking a lil bit to find the file")
        print(f"Message in TTS channel: {messageText}")
        source = discord.FFmpegPCMAudio(
        "output.wav",
        executable=r".\bin\ffmpeg\ffmpeg.exe"
        )
...
@bot.tree.command(name="tts", description="i maghi più saggi sono coloro che parlano attraverso l'Etere")
async def tts(interaction: discord.Interaction):
    await interaction.response.defer()
    ...
    await interaction.followup.send(f"L'Etere parlerà per i mortali in {interaction.channel}")
